...
while 1:
    ...
    new_data_port=UDP_PORT_NO+1
    socket_syn.sendto(("SYN-ACK"+str(UDP_PORT_NO+1)).encode(), addrclt)
    rtt1=time.time()
    (message, addrclt) = socket_syn.recvfrom(3)
    rtt2=time.time()
    message= message.decode()
    socket_data.bind((UDP_IP_ADDRESS, new_data_port))
    ...
    current_segment=1
    sliding_window= 50 # paramètre à changer pour les test
    list_ack_received=[]
